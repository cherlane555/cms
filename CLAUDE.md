...
**CMS** — a full-stack admin/content-management app generated from an existing SQL Server
schema. Backend is a .NET 9 Web API (Dapper, no EF); frontend is an Angular 20 app (standalone
components, PrimeNG). Everything lives under `src/`.

The first implemented feature is **AppRole** CRUD (list/filter, view, add, edit). New features
follow the same patterns — see "Code-generation patterns" below and `spec/code-gen.convention.md`.
...
> PrimeNG must stay on **v20** — Angular is on 20, and `primeng@latest` (v21) requires Angular 21
> and will break `npm install`. Pin `primeng@^20` / `@primeng/themes@^20`.
...
Follow `spec/code-gen.convention.md`. Summary of what the AppRole feature established:
